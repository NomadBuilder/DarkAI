"""Caching utilities to avoid redundant API calls and enrichment operations."""
...
# Try to import Config - handle both relative and absolute imports
try:
    ...
    try:
        ...
        # Fallback: create a simple Config class
        class Config:
            CACHE_ENABLED = True
            CACHE_TTL_HOURS = 24
...
def get_cached(entity_type: str, value: str, ttl_hours: int = None) -> Optional[Dict]:
    ...
    Args:
        entity_type: Type of entity (e.g., 'domain')
        value: Entity value (e.g., 'example.com')
...
def clear_cache(entity_type: Optional[str] = None):
    ...
    if entity_type:
        # Clear only entries for this entity type
        keys_to_remove = []
        for key in _cache.keys():
            # We'd need to store entity_type with the cache to filter properly
