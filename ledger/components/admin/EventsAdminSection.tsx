...
export default function AdminEventsPage({ embedded = false }: { embedded?: boolean }) {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        if (res.status === 503) {
          ...
          setSaveError(msg || 'Save must be sent from protectont.ca/admin.')
        } else {
  ...
  const main = (
      <main className={embedded ? 'pb-8' : 'max-w-3xl mx-auto px-4 sm:px-6 md:px-8 py-8 sm:py-10'}>
        {loadStatus === 'loading' && (
        ...
        {!embedded && (
          <p className="mt-4 text-center pb-8">
            <Link href="/admin" className="text-sm text-slate-500 hover:text-slate-800 font-light underline underline-offset-2">
              ← All admin
            </Link>
          </p>
        )}
  ...
  const stats =
    loadStatus !== 'loading' && events.length > 0 ? (
      <div className="flex flex-wrap gap-2 mt-4">
        <span className="inline-flex items-center rounded-full bg-violet-100 border border-violet-200 text-violet-900 px-3 py-1 text-sm font-light">
          {events.length} total
        </span>
        <span className="inline-flex items-center rounded-full bg-emerald-100 border border-emerald-200 text-emerald-900 px-3 py-1 text-sm font-light">
          {visibleCount} live on site
        </span>
        {needsDateFix > 0 && (
          <span className="inline-flex items-center rounded-full bg-amber-100 border border-amber-200 text-amber-900 px-3 py-1 text-sm font-light">
