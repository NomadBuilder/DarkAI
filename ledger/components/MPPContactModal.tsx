...
export default function MPPContactModal({ isOpen, onClose, variant = 'default' }: MPPContactModalProps) {
  ...
  const [messageBody, setMessageBody] = useState(() => getMppMessageBody(variant))

  useEffect(() => {
    if (isOpen) {
      setMessageBody(getMppMessageBody(variant))
    }
  }, [variant, isOpen])
...
${userName || '[Your Name]'}`
  ...
  const handleOpenEmail = () => {
    const subject = getMppEmailSubject(variant)
                    ...
                    <textarea
                      ...
                      onChange={(e) => {
                        ...
                        if (greetingIndex >= 0 && greetingIndex < lines.length - 1) {
                          ...
                          const regardsIndex = lines.findIndex(
                            (line, idx) => idx >= bodyStart && line.startsWith('Best regards')
                          )
                          if (regardsIndex >= 0) {
